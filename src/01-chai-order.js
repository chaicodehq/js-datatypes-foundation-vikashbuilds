...
export function whisperChaiOrder(order) {
  ...
  let secretlyOrder = order.trim();
  if (secretlyOrder.length === 0){
    return "";
  }
  return secretlyOrder.toLowerCase();
}
...
export function getFirstAndLastChar(order) {
  ...
  let firstChar = trimOrder.charAt(0);
  let lastChar = trimOrder.at(-1);

  return {
    first: trimOrder.charAt(0),
      last: trimOrder.at(-1)
    }
}
